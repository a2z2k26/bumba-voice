"""
Configuration and shared utilities for Bumba Voice Server.

This module contains all configuration constants, global state, initialization functions,
and shared utilities used across the Bumba Voice server.
"""
...
def load_bumba_env():
    """Load configuration from bumba.env file if it exists, creating a default if not."""
    # Try new filename first
    config_path = Path.home() / ".bumba" / "bumba.env"
    ...
    if not config_path.exists():
        ...
        if old_path.exists():
            ...
            print(f"Warning: Using deprecated voicemode.env - please rename to bumba.env and move to ~/.bumba/")
    
    if not config_path.exists():
        # Create default template
        config_path.parent.mkdir(parents=True, exist_ok=True)
        default_config = '''# Bumba Voice Configuration File
...
# Base directory for all Bumba Voice data (default: ~/.bumba)
# BUMBA_BASE_DIR=~/.bumba
...
# BUMBA_SAVE_AUDIO=false

# Save transcription files (true/false)
# BUMBA_SAVE_TRANSCRIPTIONS=false
...
# BUMBA_TTS_BASE_URLS=http://127.0.0.1:8880/v1,https://api.openai.com/v1

# Comma-separated list of STT endpoints
# BUMBA_STT_BASE_URLS=http://127.0.0.1:2022/v1,https://api.openai.com/v1

# Comma-separated list of preferred voices
# BUMBA_TTS_VOICES=af_sky,alloy

# Comma-separated list of preferred models
# BUMBA_TTS_MODELS=tts-1,tts-1-hd,gpt-4o-mini-tts

# Prefer local providers over cloud (true/false)
# BUMBA_PREFER_LOCAL=true

# Always attempt local providers (true/false)
# BUMBA_ALWAYS_TRY_LOCAL=true

# Auto-start Kokoro service (true/false)
# BUMBA_AUTO_START_KOKORO=false
...
# BUMBA_WHISPER_MODEL=large-v2

# Whisper server port (default: 2022)
# BUMBA_WHISPER_PORT=2022

# Language for transcription (auto, en, es, fr, de, it, pt, ru, zh, ja, ko, etc.)
# BUMBA_WHISPER_LANGUAGE=auto

# Path to Whisper models
# BUMBA_WHISPER_MODEL_PATH=~/.bumba/models/whisper
...
# BUMBA_KOKORO_PORT=8880

# Directory for Kokoro models
# BUMBA_KOKORO_MODELS_DIR=~/.bumba/models/kokoro

# Directory for Kokoro cache
# BUMBA_KOKORO_CACHE_DIR=~/.bumba/cache/kokoro

# Default Kokoro voice
# BUMBA_KOKORO_DEFAULT_VOICE=af_sky
...
# BUMBA_PTT_MACOS_ACCESSIBILITY_CHECK=true
'''
...
BASE_DIR = expand_path(os.getenv("BUMBA_BASE_DIR", str(Path.home() / ".bumba")))
...
MODELS_DIR = expand_path(os.getenv("BUMBA_MODELS_DIR", str(BASE_DIR / "models")))

# Debug configuration
DEBUG = os.getenv("BUMBA_DEBUG", "").lower() in ("true", "1", "yes", "on")
TRACE_DEBUG = os.getenv("BUMBA_DEBUG", "").lower() == "trace"
VAD_DEBUG = os.getenv("BUMBA_VAD_DEBUG", "").lower() in ("true", "1", "yes", "on")
DEBUG_DIR = LOGS_DIR / "debug"  # Debug files now go under logs

# Master save-all configuration
SAVE_ALL = os.getenv("BUMBA_SAVE_ALL", "").lower() in ("true", "1", "yes", "on")

# Audio saving configuration
# Enable if SAVE_ALL is true, DEBUG is true, or individually enabled
SAVE_AUDIO = SAVE_ALL or DEBUG or os.getenv("BUMBA_SAVE_AUDIO", "").lower() in ("true", "1", "yes", "on")
SAVE_TRANSCRIPTIONS = SAVE_ALL or DEBUG or os.getenv("BUMBA_SAVE_TRANSCRIPTIONS", "").lower() in ("true", "1", "yes", "on")

# Audio feedback configuration
AUDIO_FEEDBACK_ENABLED = os.getenv("BUMBA_AUDIO_FEEDBACK", "true").lower() in ("true", "1", "yes", "on")

# Skip TTS configuration (skip text-to-speech for faster responses)
SKIP_TTS = os.getenv("BUMBA_SKIP_TTS", "false").lower() in ("true", "1", "yes", "on")

# Local provider preference configuration
PREFER_LOCAL = os.getenv("BUMBA_PREFER_LOCAL", "true").lower() in ("true", "1", "yes", "on")

# Always try local providers (don't mark them as permanently unavailable)
ALWAYS_TRY_LOCAL = os.getenv("BUMBA_ALWAYS_TRY_LOCAL", "true").lower() in ("true", "1", "yes", "on")

# Use simple failover without health checks
SIMPLE_FAILOVER = os.getenv("BUMBA_SIMPLE_FAILOVER", "true").lower() in ("true", "1", "yes", "on")

# Auto-start configuration
AUTO_START_KOKORO = os.getenv("BUMBA_AUTO_START_KOKORO", "").lower() in ("true", "1", "yes", "on")
...
TTS_BASE_URLS = parse_comma_list("BUMBA_TTS_BASE_URLS", "http://127.0.0.1:8880/v1,https://api.openai.com/v1")
STT_BASE_URLS = parse_comma_list("BUMBA_STT_BASE_URLS", "http://127.0.0.1:2022/v1,https://api.openai.com/v1")
TTS_VOICES = parse_comma_list("BUMBA_TTS_VOICES", "af_sky,alloy")
TTS_MODELS = parse_comma_list("BUMBA_TTS_MODELS", "tts-1,tts-1-hd,gpt-4o-mini-tts")

# Legacy variables have been removed - use the new list-based configuration:
# - Bumba Voice_TTS_BASE_URLS (comma-separated list)
# - Bumba Voice_STT_BASE_URLS (comma-separated list)
# - Bumba Voice_TTS_VOICES (comma-separated list)
# - Bumba Voice_TTS_MODELS (comma-separated list)
...
WHISPER_MODEL = os.getenv("BUMBA_WHISPER_MODEL", "base")
WHISPER_PORT = int(os.getenv("BUMBA_WHISPER_PORT", "2022"))
WHISPER_LANGUAGE = os.getenv("BUMBA_WHISPER_LANGUAGE", "auto")
WHISPER_MODEL_PATH = expand_path(os.getenv("BUMBA_WHISPER_MODEL_PATH", str(Path.home() / ".bumba" / "services" / "whisper" / "models")))

# ==================== KOKORO CONFIGURATION ====================

# Kokoro-specific configuration
KOKORO_PORT = int(os.getenv("BUMBA_KOKORO_PORT", "8880"))
KOKORO_MODELS_DIR = expand_path(os.getenv("BUMBA_KOKORO_MODELS_DIR", str(BASE_DIR / "models" / "kokoro")))
KOKORO_CACHE_DIR = expand_path(os.getenv("BUMBA_KOKORO_CACHE_DIR", str(BASE_DIR / "cache" / "kokoro")))
KOKORO_DEFAULT_VOICE = os.getenv("BUMBA_KOKORO_DEFAULT_VOICE", "af_sky")

# ==================== LIVEKIT CONFIGURATION ====================

# LiveKit-specific configuration
LIVEKIT_PORT = int(os.getenv("BUMBA_LIVEKIT_PORT", "7880"))
LIVEKIT_URL = os.getenv("LIVEKIT_URL", f"ws://localhost:{LIVEKIT_PORT}")
LIVEKIT_API_KEY = os.getenv("LIVEKIT_API_KEY", "devkey")
LIVEKIT_API_SECRET = os.getenv("LIVEKIT_API_SECRET", "secret")

# LiveKit Frontend configuration
FRONTEND_HOST = os.getenv("BUMBA_FRONTEND_HOST", "127.0.0.1")
FRONTEND_PORT = int(os.getenv("BUMBA_FRONTEND_PORT", "3000"))

# ==================== SERVICE MANAGEMENT CONFIGURATION ====================

# Auto-enable services after installation
SERVICE_AUTO_ENABLE = env_bool("BUMBA_SERVICE_AUTO_ENABLE", False)
...
DISABLE_SILENCE_DETECTION = os.getenv("BUMBA_DISABLE_SILENCE_DETECTION", "false").lower() in ("true", "1", "yes", "on")

# VAD (Voice Activity Detection) configuration
VAD_AGGRESSIVENESS = int(os.getenv("BUMBA_VAD_AGGRESSIVENESS", "2"))  # 0-3, higher = more aggressive
SILENCE_THRESHOLD_MS = int(os.getenv("BUMBA_SILENCE_THRESHOLD_MS", "1000"))  # Stop after 1000ms (1 second) of silence
MIN_RECORDING_DURATION = float(os.getenv("BUMBA_MIN_RECORDING_DURATION", "0.5"))  # Minimum 0.5s recording
VAD_CHUNK_DURATION_MS = 30  # VAD frame size (must be 10, 20, or 30ms)
INITIAL_SILENCE_GRACE_PERIOD = float(os.getenv("BUMBA_INITIAL_SILENCE_GRACE_PERIOD", "1"))  # No initial silence grace period by default

# Default listen duration for converse tool
DEFAULT_LISTEN_DURATION = float(os.getenv("BUMBA_DEFAULT_LISTEN_DURATION", "120.0"))  # Default 120s listening time

# Audio feedback pip/chime configuration
# Leading silence before pips to allow Bluetooth devices to wake up
PIP_LEADING_SILENCE = float(os.getenv("BUMBA_PIP_LEADING_SILENCE", "0.1"))  # Default 0.1s - minimal delay for Bluetooth
# Trailing silence after pips to prevent cutoff
PIP_TRAILING_SILENCE = float(os.getenv("BUMBA_PIP_TRAILING_SILENCE", "0.2"))  # Default 0.2s - reduced for responsiveness

# Audio format configuration
AUDIO_FORMAT = os.getenv("BUMBA_AUDIO_FORMAT", "pcm").lower()
TTS_AUDIO_FORMAT = os.getenv("BUMBA_TTS_AUDIO_FORMAT", "pcm").lower()  # Default to PCM for optimal streaming
# STT requires a format supported by the STT provider - PCM is not supported by OpenAI Whisper
STT_AUDIO_FORMAT = os.getenv("BUMBA_STT_AUDIO_FORMAT", "mp3" if AUDIO_FORMAT == "pcm" else AUDIO_FORMAT).lower()
...
OPUS_BITRATE = int(os.getenv("BUMBA_OPUS_BITRATE", "32000"))  # Default 32kbps for voice
MP3_BITRATE = os.getenv("BUMBA_MP3_BITRATE", "64k")  # Default 64kbps
AAC_BITRATE = os.getenv("BUMBA_AAC_BITRATE", "64k")  # Default 64kbps

# ==================== STREAMING CONFIGURATION ====================

# Streaming playback configuration
STREAMING_ENABLED = os.getenv("BUMBA_STREAMING_ENABLED", "true").lower() in ("true", "1", "yes", "on")
STREAM_CHUNK_SIZE = int(os.getenv("BUMBA_STREAM_CHUNK_SIZE", "4096"))  # Download chunk size
STREAM_BUFFER_MS = int(os.getenv("BUMBA_STREAM_BUFFER_MS", "150"))  # Initial buffer before playback
STREAM_MAX_BUFFER = float(os.getenv("BUMBA_STREAM_MAX_BUFFER", "2.0"))  # Max buffer in seconds

# ==================== EVENT LOGGING CONFIGURATION ====================

# Event logging configuration
# Event logs are enabled by default, or if SAVE_ALL is true
EVENT_LOG_ENABLED = SAVE_ALL or os.getenv("BUMBA_EVENT_LOG_ENABLED", "true").lower() in ("true", "1", "yes", "on")
EVENT_LOG_DIR = os.getenv("BUMBA_EVENT_LOG_DIR", str(LOGS_DIR / "events"))
EVENT_LOG_ROTATION = os.getenv("BUMBA_EVENT_LOG_ROTATION", "daily")  # Currently only daily is supported

# ==================== PUSH-TO-TALK (PTT) CONFIGURATION ====================

# Core PTT settings
PTT_ENABLED = env_bool("BUMBA_PTT_ENABLED", False)
PTT_KEY_COMBO = os.getenv("BUMBA_PTT_KEY_COMBO", "option_r")
PTT_MODE = os.getenv("BUMBA_PTT_MODE", "hold")  # hold, toggle, or hybrid
PTT_TIMEOUT = float(os.getenv("BUMBA_PTT_TIMEOUT", "120.0"))

# Behavior settings
PTT_AUTO_ENABLE = env_bool("BUMBA_PTT_AUTO_ENABLE", False)
PTT_REQUIRE_BOTH_KEYS = env_bool("BUMBA_PTT_REQUIRE_BOTH_KEYS", True)
PTT_RELEASE_DELAY = float(os.getenv("BUMBA_PTT_RELEASE_DELAY", "0.1"))
PTT_MIN_DURATION = float(os.getenv("BUMBA_PTT_MIN_DURATION", "0.5"))

# Audio feedback settings
PTT_AUDIO_FEEDBACK = env_bool("BUMBA_PTT_AUDIO_FEEDBACK", True)
PTT_FEEDBACK_VOLUME = float(os.getenv("BUMBA_PTT_FEEDBACK_VOLUME", "0.7"))
PTT_VISUAL_FEEDBACK = env_bool("BUMBA_PTT_VISUAL_FEEDBACK", True)
PTT_VISUAL_STYLE = os.getenv("BUMBA_PTT_VISUAL_STYLE", "compact")  # minimal, compact, detailed
PTT_SHOW_DURATION = env_bool("BUMBA_PTT_SHOW_DURATION", True)

# Advanced settings
PTT_CANCEL_KEY = os.getenv("BUMBA_PTT_CANCEL_KEY", "escape")
PTT_BUFFER_PRE_RECORDING = env_bool("BUMBA_PTT_BUFFER_PRE_RECORDING", False)
PTT_BUFFER_DURATION = float(os.getenv("BUMBA_PTT_BUFFER_DURATION", "0.5"))
PTT_KEY_REPEAT_IGNORE = env_bool("BUMBA_PTT_KEY_REPEAT_IGNORE", True)

# Platform-specific settings
PTT_MACOS_ACCESSIBILITY_CHECK = env_bool("BUMBA_PTT_MACOS_ACCESSIBILITY_CHECK", True)
PTT_WINDOWS_HOOK_TYPE = os.getenv("BUMBA_PTT_WINDOWS_HOOK_TYPE", "low_level")
PTT_LINUX_INPUT_METHOD = os.getenv("BUMBA_PTT_LINUX_INPUT_METHOD", "auto")

# Statistics settings
PTT_STATISTICS = env_bool("BUMBA_PTT_STATISTICS", True)
PTT_STATS_AUTO_EXPORT = env_bool("BUMBA_PTT_STATS_AUTO_EXPORT", False)
PTT_STATS_EXPORT_PATH = os.getenv("BUMBA_PTT_STATS_EXPORT_PATH", "")

# Debug settings
PTT_DEBUG = env_bool("BUMBA_PTT_DEBUG", False)
PTT_LOG_KEYS = env_bool("BUMBA_PTT_LOG_KEYS", False)
PTT_SIMULATE = env_bool("BUMBA_PTT_SIMULATE", False)
...
def setup_logging() -> logging.Logger:
    """Configure logging for the Bumba Voice server.
    
    Returns:
        Logger instance configured for Bumba Voice
    """
